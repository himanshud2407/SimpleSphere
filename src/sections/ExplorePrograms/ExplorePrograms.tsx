    ...
    >
          ...
          {program.badges?.map((badge: string, bIdx: number) => (
            <span
              ...
              className={`px-2 py-0.5 rounded text-[10px] font-black text-white ${
                badge.includes("PLACEMENT")
                  ? "bg-purple-600"
                  : badge.includes("NCVET")
                    ? "bg-green-600"
                    : badge.includes("SKILL-UP")
                      ? "bg-indigo-600"
                      : "bg-purple-800"
              }`}
            >
      ...
      <div className="p-5 flex flex-col flex-1">
        ...
        <p className="text-slate-500 text-xs font-medium mb-6 line-clamp-2">
          {program.description ||
            `Master ${program.title} with expert instruction.`}
        </p>

        <div className="mt-auto mb-6 space-y-2">
          <div className="flex items-center gap-2 text-[10px] font-bold text-slate-600">
            <span className="text-blue-500 italic">↗</span> Price:{" "}
            {program.price || "Contact for Price"}
          </div>
          <div className="flex items-center gap-2 text-[10px] font-bold text-slate-600">
            <Award className="w-3 h-3 text-purple-500" /> Instructor:{" "}
            {program.inst || "Industry Expert"}
          </div>
        </div>
            ...
            {CATEGORIES.map((cat) => (
              ...
              >
                <div className="flex items-center gap-3">
                  <cat.icon
                    className={`w-5 h-5 ${activeTab === cat.id ? "text-blue-600" : "text-slate-400 group-hover:text-blue-500"}`}
                  />
                ...
                <div className="flex items-center gap-2">
                  <span
                    className={`px-2 py-0.5 rounded-full text-[10px] font-black ${activeTab === cat.id ? "bg-blue-600 text-white" : "bg-slate-100 text-slate-400"}`}
                  >
                  ...
                  <div className="col-span-full py-20 text-center">
                    <div className="bg-slate-50 rounded-2xl p-10 border-2 border-dashed border-slate-200">
                      ...
                      <p className="text-slate-400 text-sm">
                        We're working on exciting new content for this category.
                      </p>
                    </div>
                  </div>
                )}
        ...
        <div className="lg:hidden space-y-4">
          {CATEGORIES.map((cat) => (
            <div key={cat.id} className="overflow-hidden">
              <button
                onClick={() =>
                  setExpandedMobile(expandedMobile === cat.id ? null : cat.id)
                }
              ...
              >
                <div className="flex items-center gap-3">
                  <div
                    className={`p-2 rounded-xl transition-colors ${expandedMobile === cat.id ? "bg-[#2563EB] text-white" : "bg-slate-100 text-slate-500"}`}
                  >
                    <cat.icon className="w-5 h-5" />
                  </div>
                  <span
                    className={`font-black text-sm ${expandedMobile === cat.id ? "text-[#2563EB]" : "text-slate-700"}`}
                  >
                    {cat.name}
                  </span>
                  <span
                    className={`px-2 py-0.5 rounded-full text-[10px] font-black ${expandedMobile === cat.id ? "bg-[#2563EB] text-white" : "bg-slate-100 text-slate-500"}`}
                  >
                    {coursesMap[cat.id]?.length || 0}
                  </span>
                </div>
                <div
                  className={`p-1.5 rounded-full transition-all ${expandedMobile === cat.id ? "bg-blue-100 text-blue-600 rotate-180" : "bg-slate-50 text-slate-400"}`}
                >
